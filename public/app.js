var config = {
  ...
  var ping3 = firebase.database().ref().child("torre_3/dato");
...
puntos3 = 0;
  ...
  ping.on("value", function (snaptshot) {
    ...
    ping3.on("value", function (snaptshot) {
        control3 = 1;
        var fecha = new Date();
        var horas = fecha.getHours();
        if (horas < 10) {
        ...
        hora = horas + ":" + minutos + ":" + segundos;
        if (controlUltDesconexion3 == 1) {
            reconexion3.innerHTML = hora;
            var ctrlReconexion3 = firebase.database().ref().child("torre_3/reconexion/" + dia + "/contador");
            ctrlReconexion3.on("value", function (snaptshot) {
                ctrlReconexion3 = snaptshot.val();
            });
            ctrlReconexion3++;
            firebase.database().ref("torre_3/reconexion/" + dia + "/contador").set(ctrlReconexion3);
            var mes = fecha.getMonth() + 1;
            if (mes < 10) {
                mes = "0" + mes;
            }
            ...
            dia = dias + "-" + mes + "-" + fecha.getFullYear();
            console.log(dia);
            firebase.database().ref("torre_3/reconexion/" + dia + "/" + ctrlReconexion3).set(hora);
        }
        controlUltDesconexion3 = 0;
        ping3 = snaptshot.val();
        var antena = "En Linea";
        firebase.database().ref("torre_3/antena").set(antena);
        mostrarPing3.innerHTML = "En Línea";
        if (ping3 == 1) {
            addData3(myChart3, hora, 1);
        }
        if (ping3 == 0) {
            addData3(myChart3, hora, 1);
        }
    ...
    function actualizaGrafico() {
            ...
            if (control2 > 5) {
                if (controlUltDesconexion2 == 0) {
                    ...
                    dia = dias + "-" + mes + "-" + fecha.getFullYear();
                    var ctrlDesconexion2 = firebase.database().ref().child("torre_2/desconexion/" + dia + "/contador");
            ...
            if (control3 > 5) {
                if (controlUltDesconexion3 == 0) {
                    ...
                    dia = dias + "-" + mes + "-" + fecha.getFullYear();
                    var ctrlDesconexion3 = firebase.database().ref().child("torre_3/desconexion/" + dia + "/contador");
                    ctrlDesconexion3.on("value", function (snaptshot) {
                ...
                mostrarPing3.innerHTML = "Sin Conexión";
                var fecha = new Date();
                var horas = fecha.getHours();
                if (horas < 10) {
                    horas = "0" + horas;
                ...
                hora = horas + ":" + minutos + ":" + segundos;
                addData3(myChart3, hora, 0);
                datos3.innerHTML = hora;
            }
...
var myChart3 = new Chart(ctx, {
    ...
    options: {
        title: {
            display: true,
            text: 'Torre 3 - Red Nicolas Bravo'
        },
        scales: {
            yAxes: [{
                ticks: {
